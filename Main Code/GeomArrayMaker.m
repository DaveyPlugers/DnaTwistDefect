...
%   Mode 3 static raise and twist with single index shift, mode 4 is 2 variable raise and twists with single 
%   index shift, this is used with the reversedDefect since we have moved our amino's by one too
...
%these unless I parse them through or redefine
Gamma = 4.46;
Theta_Twist = 35.575;
Raise = 3.4; %Angstrom
Phase_Shift = -6*pi/20;
...
elseif Mode==3
    ...
    GeometricArray = zeros(N,6);
    for i=1:N
        GeometricArray(i,:) = [0,0,Raise,Theta_Twist,Gamma*cos(2*pi*(i-1)/10-Phase_Shift),Gamma*sin(2*pi*(i-1)/10-Phase_Shift)];
    end
...
elseif Mode==4
    ...
    GeometricArray = zeros(N,6);
    for i=1:N
        GeometricArray(i,:) = [0,0,Geometric_Properties(DNAIndexation(i),DNAIndexation(i+1),7),Geometric_Properties(DNAIndexation(i),DNAIndexation(i+1),5),Gamma*cos(2*pi*(i-1)/10-Phase_Shift),Gamma*sin(2*pi*(i-1)/10-Phase_Shift)];
    end
end
