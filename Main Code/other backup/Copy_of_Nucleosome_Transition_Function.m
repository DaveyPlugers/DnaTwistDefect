function [Introduction_Time,Leave_Time,k,Time] = Copy_of_Nucleosome_Transition_Function(Duration,DNA,Position_Remodeller,Start_Position,Chr_Dir,IntroductionRate,Variable_Introrate,Passable_Chromatin,Chromatin_Barrier,Barrier_Left,Barrier_Right)
...
k = Start_Position + (-1)^(Chr_Dir+1) ; %what position does our simulation start at, +-1 due to the need for a first defect to be introduced
%must be undone in the end
...
while Looping
    ...
    for y=1:n %We put extra conditions on the movement to see if we have any movement there, if not we skip it saving time since the calculation of l is a bottleneck
        if y==1&&(Right_Movement(1)==1||(Passable_Chromatin&&Defect_Array(1)==1))    %Chr_Dir*M gives 0 or M which denotes left or right, mod(k-1)+1 makes our position between 1 and M + our previous from Chr_Dir
            Right_Movement(y) = Right_Movement(y)*exp(-(Barrier_Right(Chr_Dir*M+mod(k-1,M)+1,p)));
            %p+1, p is because we have our chrom. at position 13-n=p
        ...
        elseif y==n&&(Left_Movement(y)==1||Right_Movement(y)==1)
            if Chr_Dir == 0%2 directions work differently for our new index
                l = mod(k + sum(Defect_Array(1:n-1))-1,M)+1; %Double check
            else
            ...
            %Right_Movement(y) = Defect_Array(y)*exp(-(Barrier_Right(Chr_Dir*M+l,13))); %Note this is the leaving one, we may want to adjust more here later
            Right_Movement(y) = Defect_Array(y)*exp(-3);
            TransitionRate = TransitionRate + Left_Movement(y) + Right_Movement(y);
        elseif (Left_Movement(y)==1||Right_Movement(y)==1)
            if Chr_Dir == 0 %2 directions; works differently for our new index
                l = mod(k + sum(Defect_Array(1:y-1))-1,M)+1; %Double check
            else
    ...
    elseif TransitieCoefficient < Introduction_Possible*IntroductionRate
        Defect_Array(1) = Defect_Array(1) + 1; %Here we introduce a twist, doing like this made it possible to test we never overlap and don't see any 2's
        if Chr_Dir ==0
            k = mod(k-2,M) +1; %k -> k-1
        else
            k = mod(k,M) +1; %k -> k+1
